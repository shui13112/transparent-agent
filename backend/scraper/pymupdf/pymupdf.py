...
class PyMuPDFScraper:
    ...
    def __init__(self, link, session=None, **_kwargs):
        """
    ...
    def scrape(self) -> tuple[str, str]:
        ...
        and returns the document content and title.
        ...
        try:
            if self.is_url():
                try:
                    response = requests.get(self.link, timeout=(5, 15), stream=True)
                    ...
                    response = requests.get(self.link, timeout=(5, 15), stream=True, verify=False)
                    response.raise_for_status()
