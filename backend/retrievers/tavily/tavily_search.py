"""Tavily API搜索检索器。
该模块提供TavilySearch类，用于通过Tavily API执行网络搜索。
"""
...
class TavilySearch:
    ...
    def get_api_key(self):
        """
        Gets the Tavily API key from headers or environment variable.
        """
        ...
        if not api_key:
            try:
                ...
                logger.warning(
                    "Tavily API key not found. If you need a retriever, please set the TAVILY_API_KEY environment variable."
                )
                return ""
        return api_key
