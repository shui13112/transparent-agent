...
export async function getWebContent(url: string, sessionId: string) {
  return request<WebContent>(
    `/web-content?url=${encodeURIComponent(url)}&session_id=${encodeURIComponent(sessionId)}`
  );
}
